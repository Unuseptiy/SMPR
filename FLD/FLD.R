...
sigma_cnt <- function(feature_matrix, mu, class_quan) {
  l <- dim(feature_matrix)[1]
  n <- length(mu)
  sum <- matrix(0, n, n)
  for (i in 1:l) {
...
FLD <- function (z, feature_matrix, labels, class_quan, lambda, sigma) {
  l <- dim(feature_matrix)[1]
  n <- dim(feature_matrix)[2]
  p <- rep(0, class_quan)
  E <- matrix(0, 0, n)
...
#z <- c(3, 0.7)
#FLD(z, as.matrix(iris[,3:4]), iris[,5], 3, c(1, 1, 1))

draw <- function() {
  colors <- c("setosa" = "red", "versicolor" = "green3", "virginica" = "blue")
  plot(iris[,3:4], pch = 21, col = colors[iris[,5]], bg = colors[iris[,5]], main = "Карта классификации ирисов Фишера\nc помощью линейного дискриминанта Фишера\nlambda_1 = lambda_2 = lambda_3 = 1")
  ...
  for (x in seq(l, r, deltaX)) {
    for (y in seq(b, t, deltaY)) {
      z <- c(x, y)
      class <- FLD(z, as.matrix(iris[,3:4]), iris[,5], 3, c(1, 1, 1))
