...
gamma_set <- function(feature_matrix, labels, eps) {
  l <- dim(feature_matrix)[1]
  gamma <- rep(0, l)
  h <- rep(0.4, l)
  loo <- eps + 1
  ...
  while(loo >= eps & cnt < 5) {
    loo <- 0
    for (i in 1:l){
      pot <- poten(feature_matrix[-i,], labels[-i], feature_matrix[i,], tria, h, gamma)
...
plot(iris[,3:4], pch = 21, bg = tmp_colors, col = tmp_colors, xlab = "Petal.Length", ylab = "Petal.Width", main = "Карта классификации ирисов Фишера методом\nпотенциальных функций")
