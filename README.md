...
3. [Линейные алгоритмы классификации](#Линейные-алгоритмы-классификации)<br>
    3.1. [ADALINE, правило Хэба](#ADALINE,-правило-Хэба)<br>
...
Функцией потерь в данном классе алгоритмов считается некоторая мажоранта [M < 0]. 
...
ADALINE <- function (feature_matrix, labels, L, flag, eps) {
  ...
  while (Q > eps & cnt < 5) {
    ...
    if (check %% 50 == 0) print(c(epsilon, Q))
