...
Так же разделяющие плоскости могут быть нелинейны, это бывает в случаях, когда
ковариационные матрицы классов не диагональны и не равны. В таких случаях
разделяющая плоскость может принимать вид:

1) Параболический (дисперсии "синих" больше дисперсий "зеленых"):
![](plug-in/parabolic.png)
...
Когда требуется решить к какому из ДВУХ линейно разделимых классов {-1, 1}
отностится объект применяют линейные алгоритмы классификации. Модель этих
алгоритмов: a(x) = sign(f(x, w)), f(x) - дискриминантная функция.

Отступом объекта x относительного данного алгоритма классификации называется:
M(w) = y * f(x, w), где у - класс объекта х.

Функцией потерь L в данном классе алгоритмов считается некоторая мажоранта [M < 0].

### Стохастический градиентный спуск

[Оглавление](#Оглавление)

В задачах обучения линейных классификаторов нужно подбирать вектор параметров, 
на котором минимизируется функция потерь ∑ L, где сумма берется по всем
элементам обучающей выборки. Эти веса подбираются с помощью метода
стохастического градиентного спуска.

Почему градиентный спуск? Как известно функция быстрее всего убыват в
направлении своего антиградиента. Поэтому мы используем градиентный спуск,
чтобы подобрать веса, минимизирующие функцию потерь.

Почему стохастический?
Если предъявлять объекты в некотором случайном порядке, то получается метод
стохастического градиентного спуска (Stochastic Gradient Descent - SGD).

Функция ***SGD*** реализует метод стохастического градиентного спуска
для произвольной функции потерь. Вход: матрица признаков, вектор меток, 
функция потерь, пеовая производная функции потерь, максимальная величина ошибки.
Выход: вектор весов.
